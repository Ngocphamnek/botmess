     ...
     def connect_mqtt(self):
          ...
          def on_message(client, userdata, msg):
               try:
                    ...
                    print(f"[{datetime.datetime.now()}][mqtt] topic={msg.topic} keys={list(j.keys())} payload_preview={str(j)[:200]}")
                    deltas = j.get("deltas")
                    if isinstance(deltas, list):
                         print(f"[{datetime.datetime.now()}][mqtt] {len(deltas)} delta(s) received")
                         for delta in deltas:
                              print(f"[{datetime.datetime.now()}][mqtt] delta class={delta.get('class')} keys={list(delta.keys())}")
                              body = self._body_from_delta(delta)
                              if body is not None:
                                   print(f"[{datetime.datetime.now()}][mqtt] parsed body={body}")
                                   self._publish_body_results(body)
                              else:
                                   print(f"[{datetime.datetime.now()}][mqtt] delta skipped (no messageMetadata)")
